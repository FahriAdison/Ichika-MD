...
exports.WAConnection = _WAConnection => {
  class WAConnection extends _WAConnection {
    ...
    async sendMediaAsSticker(jid, path, quoted, options = {}){
	let { ext, mime, data } = await this.getFile(path)
	let media = {}
let buffer
media.data = data
	media.mimetype = mime
...
buffer = /image/.test(mime) ? await imageToWebp(data) : /video/.test(mime) ? await videoToWebp(data) : ""
	...
	await this.sendMessage(jid, buffer, MessageType.sticker, { quoted, ...options })
return buffer
}
    ...
    async sendVideoAsSticker(jid, path, quoted, options) {
        ...
        async sendImageAsSticker(jid, stickerData, quoted, options = {}) {
			let packname = options.packname || global.packname
			...
			.then(res => {
				...
				this.sendMessage(jid, imageBuffer, MessageType.sticker, { quoted: quoted })
		...
		async sendMp4AsSticker(jid, stickerData, quoted, options = {}) {
			...
			.then(res => {
				let imageBuffer = new Buffer.from(res.split(';base64,')[1], 'base64')
				this.sendMessage(jid, imageBuffer, MessageType.sticker, { quoted: quoted })
			}).catch(e => this.logger.error(e))
		}
